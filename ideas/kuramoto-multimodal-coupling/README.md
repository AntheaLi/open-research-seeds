...
**Setup**: Take a standard vision-language backbone (e.g., ViT + text transformer) and attach a phase-mapping head to each encoder — a small MLP that maps each modality's embedding to oscillator phases (a vector in [0, 2π)^d where d is the number of oscillators per modality).
