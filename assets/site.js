(function () {
  ...
  function renderSeedCards() {
    seedGrid.innerHTML = seeds.map(function (seed) {
      var tags = seed.tags.map(function (tag) {
        return '<span>' + escapeHtml(tag) + '</span>';
      }).join('');
      var active = activeSeed && activeSeed.slug === seed.slug ? ' active' : '';
      return [
        '<button class="seed-card' + active + '" type="button" data-slug="' + escapeHtml(seed.slug) + '">',
        '<span class="seed-ramp">' + escapeHtml(seed.rampUp) + ' ramp-up</span>',
        '<strong>' + escapeHtml(seed.title) + '</strong>',
        '<span class="seed-hook">' + escapeHtml(seed.hook) + '</span>',
        '<span class="seed-background">' + escapeHtml(seed.background) + '</span>',
  ...
  function selectSeed(slug, view) {
    var nextSeed = seeds.filter(function (seed) { return seed.slug === slug; })[0] || seeds[0];
    activeSeed = nextSeed;
  ...
  fetch('seeds.json')
    ...
    .then(function (data) {
      ...
      activeSeed = seeds.filter(function (seed) { return seed.slug === requestedSlug; })[0] || seeds[0];
      renderSeedCards();
      selectSeed(activeSeed.slug, 'overview');
    })
    .catch(function () {
